...
const defaults: PushStorage = {
  ...
  push_lang: {
    ...
    push_message_error: 'Loopinuz is syncing in the background...',
...
async function handlePushNotificationObject(obj: PushNotificationObject) {
  ...
  try {
    ...
    const tag = 'fix';
    const notificationPromise = ctx.registration.showNotification('Loopinuz Web', {
